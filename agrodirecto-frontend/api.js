...
async function obtenerFavoritos(compradorId) {
    return peticion(`/favoritos?comprador_id=${compradorId}`);
}

async function agregarFavorito(compradorId, productorId) {
    return peticion("/favoritos", {
        method: "POST",
        body: JSON.stringify({ comprador_id: compradorId, productor_id: productorId }),
    });
}

async function eliminarFavoritoApi(compradorId, productorId) {
    return peticion(`/favoritos/${compradorId}/${productorId}?confirmar=true`, {
        method: "DELETE",
    });
}
...
// Envia un mensaje de soporte (se guarda en la tabla "soporte" de la BD).
// datos = { usuario_id, nombre, correo, mensaje }
async function enviarSoporte(datos) {
    return peticion("/soporte", {
        method: "POST",
        body: JSON.stringify(datos),
    });
}

// Lista los tickets de soporte. Uso administrativo.
async function obtenerTicketsSoporte(filtros = {}) {
    const params = new URLSearchParams(filtros).toString();
    return peticion(`/soporte${params ? "?" + params : ""}`);
}

// Cambia el estado de un ticket (Pendiente, En proceso, Resuelto).
async function actualizarEstadoSoporte(id, estado) {
    return peticion(`/soporte/${id}/estado`, {
        method: "PUT",
...
async function eliminarTicketSoporte(id) {
    return peticion(`/soporte/${id}?confirmar=true`, {
        method: "DELETE",
    });
}
